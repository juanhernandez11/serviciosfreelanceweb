...
const MobileMenu = ({ isMenuOpen, setIsMenuOpen, navLinks }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[60] lg:hidden transition-all duration-500 ${isMenuOpen ? 'visible' : 'invisible'}`}>
      ...
      />
      <div className={`absolute right-0 top-0 bottom-0 w-[85%] max-w-sm bg-white dark:bg-slate-900 shadow-2xl transition-transform duration-500 ease-out p-8 flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center mb-12">
          <Logo className="w-8 h-8 text-slate-900 dark:text-white" />
          <button onClick={() => setIsMenuOpen(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full" aria-label="Cerrar menú">
        ...
        <div className="flex flex-col gap-8 flex-grow">
          {navLinks.map((link) => (
            <a
              ...
              className="text-2xl font-black text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 tracking-tighter"
            >
              {link.name}
            </a>
          ))}
